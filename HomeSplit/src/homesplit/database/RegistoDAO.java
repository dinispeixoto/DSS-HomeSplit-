 ...
 * @author Grupo 35
 */
public class RegistoDAO implements Map<String,Morador> {
        ...
        try{
            ...
            if(rs.next()){
               ...
               /* List<String> notificacoes = new ArrayList();
                ps = con.prepareStatement("SELECT * FROM Notificacao WHERE Morador = ? AND Estado = ?");
                ps.setString(1,(String) key);
                ps.setString(2, "registo");
                while(rs.next()){
                ...
                ps = con.prepareStatement("SELECT * FROM Solicitacao WHERE Morador_Responsavel = ? AND Estado = ?");
                ps.setString(1,(String) key);
                while(rs.next()){
